...
class ModelTrainer:
    ...
    def create(self, model_topologies):
        if isinstance(model_topologies, list) and all(
            isinstance(model, list) and all(isinstance(layer_data, dict) for layer_data in model)
        ...
        elif isinstance(model_topologies, list) and all(
            isinstance(layer_data, dict) for layer_data in model_topologies
        ):
    ...
    def train(
        ...
        if len(model_list) == 0:
            raise RuntimeError(
                "You must create your model before training. Use `create(model_topologies)"
            )
    ...
    def train_model(
        ...
        model.fit(
            x_train, y_train, validation_data=validation_data, batch_size=batch_size, epochs=epochs
        )
